...
cfg に保存されたチェイン選択を読まない。終了時には、そのとき動いているチェインとスライダー値が
素の MAME と同じく cfg に保存される。
`vbmame.ini`(101)に書いた場合は通常の既定値として扱われ、cfg の選択が優先される。
cfg を書き換えずに試したいときは `-bgfx_cfg_readonly` を付ける。
...
### `vector_list_sync` / 別名 `veclistsync`

| | |
|---|---|
| 型 / 既定 | bool / **`1`(オン)** |

**完成したベクターリストを、次の画面更新を待たずに次の present でレンダラへ渡す。**

CPU が 1 枚のリストを描き終える瞬間(AVG/DVG なら VGGO、Vectrex なら VIA T2 の
リフレッシュタイマ)と、エミュレートされた画面更新のタイミングは無関係である。
オフだと、レンダラから見た「1 パスの生存時間」は画面 refresh 周期の整数倍に丸められる。
リストの実寿命は refresh 周期の 1.18〜1.89 倍だったので、位相次第で 1 回または 2 回しか
サンプルされず、**31.5 ms の掃引に 12.5 ms しか提示時間が与えられない**ことが起きる。
一番長いリストが一番短い時間しか貰えないという逆転になり、
毎回同じストロークだけが激しくちらつく。

オンにすると丸めの単位が present 周期になる。Star Wars のランキング画面
(`beam_window_scale` 1.0、present 120 Hz)で、短いパスの欠落が 1059 本中 291 本から
43 本へ減った。`beam_window_scale` を上げて埋め合わせる必要がその分だけ減る。

`vector_beam_window` がオフ、または present ループが回っていないときは何もしない。
MVEC の録画内容は変わらない。
従来の挙動に戻すには `-noveclistsync`。
...
最大フレームレート、Linux では SDL のディスプレイモード(SDL2 は整数 Hz、`OSD=sdl3` なら
正確な値。HDR 設定のページを参照)を取得する。タイマーは検出値を小数のまま使うので、
119.879 Hz のパネルを 120 Hz で駆動することはない。検出されるまでは 60 Hz で動く。
...
レンダラより上流の中間表現なので、**同じ MVEC を別のレンダラ設定に流せば
差分はレンダラだけになる。**

**形式 1.2 以降は「パブリケーション」単位で記録する。** ライブのベクターデバイスは
リストが完成した時点で帯域外に公開することがあり、これは画面更新の刻みとは一致しない。
1.1 までは画面更新時だけを記録していたため、リストの真の周期がフレーム格子に量子化され、
再生時にパスが 1 フレーム分か 2 フレーム分かのどちらかになっていた(starwars で
present 数 4 が 395 回・8 が 210 回、その中間がゼロ)。ビームタイムウィンドウはこの
パス境界でペースを取るため、量子化をそのまま内容と解釈してちらついた。
1.2 は各パブリケーションに**公開時刻**と帯域外フラグを持たせ、再生がタイムラインを再現する。

**画面に出るフレーム番号は従来どおりフレーム公開だけを数える。** 帯域外パブリケーションは
再現のための内部表現なので番号には入らない(入れると既存のキャプチャ手順の番号が
ずれる)。1.1 以前は帯域外が存在しないので番号は完全に一致する。
...
形式 1.2 は帯域外パブリケーションも記録するので、1.1 に比べて **starwars で約 1.66 倍**になる。
...
`cfg/<game>.cfg` のチェイン選択とスライダー値をいつもどおり読むが、**書き戻さない**。
メニューで動かしたスライダー、メニューで選んだチェイン、`-bgfx_screen_chains` で指定した
チェインは、どれもマシンを終了するまでの一時的なものになり、cfg は元の内容のまま残る。
もともと cfg が無ければ何も書かれない。

設定を変えたくない実行のためのもの。別のチェインでちょっと試すとき、スクリプトや
AI エージェントにメニューを操作させるときなど。止めるのは cfg の BGFX 部分だけで、
入力割り当てや DIP スイッチなどは通常どおり保存される。
...
**優先度 105(`source/*.ini`)以上で指定すると cfg のチェイン選択を読まない。**
保存は素の MAME 0.289 と同じで、終了時に動いているチェインとスライダー値が cfg に書かれる。
`source/*.ini` でチェインを選んだゲームでも、メニューでの調整がそのまま残る。
一度テストで指定しただけで保存設定が書き換わるのを避けたいときは `-bgfx_cfg_readonly` を使う。

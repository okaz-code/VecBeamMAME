...
| **クリップ窓のサンプル&ホールド** | MAJOR HAVOC / BATTLEZONE の窓回路の非理想性を再現 | 対象ハードが Atari AVG のみなので常に有効 |
...
**カラー** — 赤ガンの中間 2 階調が回路の分圧比と食い違っていて、本来 2 色に分かれるはずの
ところが分離していませんでした。

**速度** — 画面更新レートが実機と合っていませんでした。alpha のクロックまわりを回路図どおりに
直してあります。
...
## Vectrex のオーバーレイが MINE STORM の 1 枚しか入っていません

**作るのが大変なのです。** 1 枚のオーバーレイは、表面のカラー印刷・裏面の白印刷・樹脂板の
着色・板の切り欠き形状をそれぞれ用意して組み上げる必要があり、相応の手間がかかります。

MINE STORM のオーバーレイは Vectrex の中でも一番凝った作りで、最初の 1 枚に選んだ理由も
そこにあります。

余裕ができたら残りも作りたいとは思っています。
...
1. **モニタ本体の輝度を最大に上げる。** パネルが実際に出せるピークを決めているのはこちらなので、
   ここを上げないと物理的に届きません。自動輝度調整（およびパネル側の省電力・ABL 設定）は切る
2. **macOS 側の「ディスプレイ」輝度スライダは無理に上げる必要はありません。** これはモニタ本体の
   輝度ではなく SDR 基準白の位置を動かすもので、上げても絵は明るくならず（絶対 nits 較正では
   ビームの輝度は一定で、UI だけが明るくなる）、上げすぎると EDR ヘッドルームが尽きて
   ハイライトの余地が無くなります。基準白が低いほうがヘッドルームは広く取れます
3. **HDR が有効になっているか確認する。** 有効にできないと SDR にフォールバックするので暗くなる
4. モニタのピーク輝度が自動取得できない場合だけ `bgfx_hdr_display_peak` で指定する
   （macOS は potential headroom から自動導出するので通常は不要）
...
**それが正しいからです。** ベクタースキャンでもラスタースキャンでも、CRT を**真正面から見て
いる限り画像は歪みません。** 管面がいくら球面でも、そこに描かれている絵そのものが曲がる
わけではありません。曲がって見えるのは斜めから見たときの見え方の話です。

`Tube Quadric Distortion`（管面の球面による歪み）は**ガラス面の形**を表すパラメータで、
偏向系の歪みではありません。反射やハイライト、管面の縁の丸まりに効きます。

**ビームの側を歪ませたいなら**、偏向の非線形性を表す `Vector Pincushion X (Quad)` /
`Vector Pincushion Y (Quad)` を使ってください。実機の偏向回路の調整ずれに相当するもので、
こちらは画像そのものが歪みます。

### アートワークがずれるとき

`[M] Monitor/Glass Sim` が On のとき、絵を管面の内側に収めるため `tube_face_scale` に 0.98、
`vector_image_scale` に 0.94 が掛かります。**アートワークの画面枠と合わなくなる場合は
`[M] Monitor/Glass Sim` を Off にしてください。**（シャドウマスク、周辺デフォーカス、
管面の周辺減光、ベゼル反射なども一緒に切れます。）
...
present レート・RGB 別ビームスポット径をまとめて設定します。

| | 内部解像度 | 出力解像度 | ビーム時間窓 | present レート | RGB Spot Width |
|---|---|---|---|---|---|
| `high` | 1.0 | 1.0 | オン | モニタのリフレッシュ | チェインの設定どおり |
| `medium` | 0.75 | 0.75 | オン | **60 Hz 固定** | チェインの設定どおり |
| `low` | 0.5 | 0.5 | **オフ** | なし（present ループ自体が回らない） | **オフ** |

**この中で RGB Spot Width だけは解像度で届かない項目**で、遅い GPU では単独で一番
大きいことがよくあります。ビームを1回ではなく銃ごとに3回描くので、チェインで一番重い
シェーダの仕事がストロークあたり3倍になり、描画先を小さくしても減りません。だから
`low` では切ります。較正済みのスケール値は残るので、`medium` / `high` に戻せば絵も戻ります。
...
## Mac / Linux バイナリが頒布されていないのはなぜ？

色々と事情があります。MAME 公式だって Mac / Linux 用バイナリは頒布していません。
コンパイル自体は難しくはありません（[README.md の How to compile?](../../README.md#how-to-compile)）。

## Linux で動きますか？

**動きます。** ただし**作者の環境では未確認**です。

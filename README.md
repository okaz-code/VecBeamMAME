...
### MAJOR HAVOC や BATTLEZONE で画面の縁が揺れる／わずかにずれる

**仕様です。** この2機種はクリップ窓の位置をサンプル&ホールド回路で保持していて、
その回路の非理想性（垂下・誘電吸収・散らばり・オフセット）を再現しています。
MAJOR HAVOC は上端（`ymin`）の1辺、BATTLEZONE は矩形の4辺すべてが対象です。**既定で有効**です。

理想的なホールドと比べたい / 切りたい場合は `-novector_window_sim` で4項目まとめてゼロになります。

→ [起動時オプション `vector_window_sim`](docs/vecbeammame/startup-options.ja.md)

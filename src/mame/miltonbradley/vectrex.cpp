...
static INPUT_PORTS_START(vectrex)
	...
	PORT_START("JDOTFIX")
	PORT_CONFNAME(0x01, 0x00, "Junction dot fix")   // detect length-0 dwell dots sitting on a line end (the split-line bulge)
	PORT_CONFSETTING(0x00, DEF_STR(Off))
	PORT_CONFSETTING(0x01, DEF_STR(On))
	PORT_START("JDOTLVL")
	PORT_ADJUSTER(0, "Junction dot level")   // when JDOTFIX on: brightness of those dwell dots (adj/100; 0 = drop, 100 = full, between = dim so per-frame persistence doesn't make the static dot pop)
	PORT_START("OBJKNEE")
	...
	PORT_ADJUSTER(25, "Text lift cutoff")    // LINE segments with BIOS vector scale >= adj*2 are treated as text and NOT lifted (game ~12-28, text ~84-132; 25 = 50, 100 = 200 = off)
	PORT_START("TEXTCAP")

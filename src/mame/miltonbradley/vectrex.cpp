...
static INPUT_PORTS_START(vectrex)
	...
	PORT_ADJUSTER(30, "Dwell accum limit")   // cap same-spot pile-up; 0 = only first dot, 100 = ~unlimited (cap = adj/100 x 16)
	...
	PORT_START("SPOTKRNG")
	PORT_ADJUSTER(12, "Spot killer range")   // beam must swing beyond this from centre to stay alive (adj %; 0 = centre only, 100 = full draw range to the edge)
	PORT_START("OBJKNEE")

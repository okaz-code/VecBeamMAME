...
static INPUT_PORTS_START(vectrex)
	...
	PORT_START("3DCONF")
	PORT_CONFNAME(0x01, 0x01, "3D Imager")
	...
	PORT_START("BEAMINFL")
	PORT_ADJUSTER(100, "Beam draw-time influence") // 0 = flat intensity, 100 = fully draw-time shaped (short=dim, long=bright)
	...
	PORT_ADJUSTER(50, "Beam draw-time scale")     // saturation midpoint (dt normalizer = adj x 30000; stronger time->brightness)
	PORT_START("BEAMMAX")
	PORT_ADJUSTER(30, "Beam max energy")          // per-unit-area phosphor saturation ceiling (max = adj/100 x 8; 50 = 4.0)
	PORT_START("BEAMDWELL")
	PORT_ADJUSTER(0, "Dwell accum limit")    // cap same-spot pile-up; 0 = only first dot, 100 = ~unlimited (cap = adj/100 x 16)
	...
	PORT_START("BEAMMODE")
	PORT_CONFNAME(0x01, 0x01, "Beam energy mode")   // discrete Off/On toggle (Machine Configuration menu), not a slider
	PORT_CONFSETTING(0x00, "Per-segment dt (legacy)")
	PORT_CONFSETTING(0x01, "RAMP-stroke aggregate")
	PORT_START("BEAMSPEED")
	PORT_ADJUSTER(50, "Stroke speed norm")   // stroke-mode inverse-speed normalizer (x = (1/speed) * adj*90000); only used when BEAMMODE>0
	PORT_START("JDOTFIX")
	PORT_CONFNAME(0x01, 0x01, "Junction dot fix")   // detect length-0 dwell dots sitting on a line end (the split-line bulge)
	PORT_CONFSETTING(0x00, DEF_STR(Off))
	PORT_CONFSETTING(0x01, DEF_STR(On))
	PORT_START("JDOTLVL")
	PORT_ADJUSTER(10, "Junction dot level")   // when JDOTFIX on: brightness of those dwell dots (adj/100; 0 = drop, 100 = full, between = dim so per-frame persistence doesn't make the static dot pop)
	PORT_START("BRIGHT")
	PORT_ADJUSTER(50, "Brightness knob")     // master beam brightness, like the real front-panel knob: 50 = normal (x1), 100 = x2 intensity
	PORT_START("BRTRETR")
	PORT_ADJUSTER(50, "Brightness retrace floor") // above ~75% brightness, lift blanked (intensity 0) beams so the retrace faintly glows (adj/100 x ~160 max, ramped in from 75 to 100)
	PORT_START("SPOTKILL")
	...
	PORT_CONFSETTING(0x01, DEF_STR(On))
	PORT_START("SPOTKMS")
	PORT_ADJUSTER(25, "Spot killer time")    // no-deflection time before the beam is cut (adj x 10 ms; 25 = 250 ms)
	PORT_START("SPOTKRNG")
	PORT_ADJUSTER(60, "Spot killer range")   // beam must swing beyond this from centre to stay alive (adj %; 0 = centre only, 100 = full draw range to the edge)
	PORT_START("OBJKNEE")
	...
	PORT_START("TEXTSCALE")
	PORT_ADJUSTER(100, "Text lift cutoff")    // LINE segments with BIOS vector scale >= adj*2 are treated as text and NOT lifted (game ~12-28, text ~84-132; 25 = 50, 100 = 200 = off)
	PORT_START("TEXTCAP")
	PORT_ADJUSTER(50, "Text energy cap")     // beam_energy ceiling for those text lines (adj/50; 50 = 1.0 = normal/no lift)

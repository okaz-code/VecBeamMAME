...
static INPUT_PORTS_START(vectrex)
	...
	PORT_START("SCRSYNC")
	PORT_CONFNAME(0x01, 0x01, "Screen refresh")   // follow the game's VIA T2 frame (one screen update per pass), or the stock fixed 60 Hz
	PORT_CONFSETTING(0x00, "Fixed 60 Hz")
	PORT_CONFSETTING(0x01, "Follow game (VIA T2)")
	PORT_START("OBJKNEE")

...
public:
	...
	float               overload = 0.0F;    // overload value for special use by renderers
...
public:
	...
	void add_line(float x0, float y0, float x1, float y1, float width, rgb_t argb, u32 flags, float overload = 0.0f);  // overload param
...
private:
	...
	public:
		item() : m_next(nullptr), m_type(0), m_flags(0), m_internal(0), m_width(0), m_overload(0), m_texture(nullptr) { }  // m_overload
	...
	private:
		...
		float               m_overload;         // overload value for special use by renderers

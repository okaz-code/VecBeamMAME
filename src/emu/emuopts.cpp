...
const options_entry emu_options::s_option_entries[] =
{
	...
	{ OPTION_VECTOR_COLOR_BLEED ";vecbleed",             "1",         core_options::option_type::BOOLEAN,    "reproduce the colour leak of generators whose dark guns are not fully cut off (Quantum): its ColorRAM pull-downs only divide the Z reference rather than ground it, so a dark gun still passes a share of it and bright vectors wash towards white. Off clamps a dark gun to black, which is what MAME did before" },
	{ OPTION_VECTOR_CLOCK_STRETCH ";vecstretch",         "1.0",       core_options::option_type::FLOAT,      "CPU clock cycles a board's vector-memory access costs the CPU, for the machines that stretch it (Major Havoc holds F0a high for one period whenever the alpha addresses vector RAM, so the circuit value is 1.0). 0 turns it off, which is what MAME did before. Fractional values accumulate, so 1.5 stretches every other access twice" },
	{ OPTION_VECTOR_VG_STALL ";vecvgstall",              "1.0",       core_options::option_type::FLOAT,      "CPU-clock periods the VECTOR GENERATOR loses per CPU vector-memory access, on the boards that arbitrate that memory. The other half of vector_clock_stretch: on Major Havoc the same STRETCHa that stretches the alpha also raises VMEMEN, which hands the address multiplexer to the alpha AND inhibits the generator's state-machine clock (SP-252 sheet 6B, 5C LS27 pin 11), so the real sweep is lengthened by the CPU's traffic while MAME's runs at a constant rate. The default 1.0 is the circuit value, one such period per access; 0 turns it off, which is what MAME did before. Fractional values accumulate" },
	{ OPTION_VECTOR_CCPU_DWELL ";ccpudwell",              "21.0",      core_options::option_type::FLOAT,      "microseconds a Cinematronics CCPU draw-vector holds the beam before it starts moving. It is what makes a degenerate vector a bright parked dot, and it is set where the renderer's two energy branches meet, so a one-pixel stroke comes out as bright as the dot beside it instead of falling off a cliff" },
	{ OPTION_VECTOR_CCPU_RATE ";ccpurate",                "1.62",      core_options::option_type::FLOAT,      "screen pixels per microsecond a Cinematronics CCPU beam sweeps once it is moving. Sets how bright a long stroke is: raise it and long strokes are swept faster and come out dimmer, lower it and they brighten. Without a length term at all a long vector is drawn in the same time as a short one and fades out" },
	...
	{ OPTION_VECTOR_LIST_SYNC ";veclistsync",             "1",         core_options::option_type::BOOLEAN,    "hand a new vector beam list to the renderer at the presentation rate instead of at the next emulated screen refresh, so a pass is presented for as long as it really lived" },
	...
	{ OPTION_VECTOR_WINDOW_JITTER,                       "1.0",       core_options::option_type::FLOAT,      "clip-window per-sample scatter, percent of screen height; constant until that edge is latched again. Needs -vector_window_scatter, which is what switches the scatter on" },
	...
	{ OPTION_VECTOR_PLAYBACK_START ";vecstart",          "0",         core_options::option_type::INTEGER,    "seek to this frame when vector playback starts, counting from 1 as the overlay does (0 = the beginning), so a run can be reproduced without driving the goto tool by hand" },
	{ OPTION_VECTOR_PLAYBACK_PAUSE ";vecpause",          "0",         core_options::option_type::BOOLEAN,    "start vector playback paused, so -vector_playback_start lands on a still frame that an external capture can be timed against" },
	{ OPTION_VECTOR_PLAYBACK_ADVANCE ";vecadvance",     "0",         core_options::option_type::INTEGER,    "play this many frames after -vector_playback_start before pausing, so the phosphor pool is warm and carries the trails that frame really follows (0 = pause immediately)" },
	{ OPTION_VECTOR_PLAYBACK_DECAY ";vecdecay",         "0",         core_options::option_type::INTEGER,    "after reaching the paused frame, advance display time by this many recorded frame periods while drawing NOTHING, so what remains is pure phosphor decay of that one frame (0 = off)" },

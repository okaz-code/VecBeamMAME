...
const options_entry emu_options::s_option_entries[] =
{
	...
	{ OPTION_VECTOR_WINDOW_SIM,                          "0",         core_options::option_type::BOOLEAN,    "clip-window scatter for the hardware that has the circuit (Major Havoc, Battlezone): the latched edge lands somewhere slightly different every frame, seen as the window wobbling. Off by default - on Battlezone all four edges are held, so the whole frame breathes. The other three imperfections (droop, dielectric absorption, offset) are always on" },
	...
	{ OPTION_VECTOR_WINDOW_JITTER,                       "1.0",       core_options::option_type::FLOAT,      "clip-window per-sample scatter, percent of screen height; constant until that edge is latched again. Needs -vector_window_sim, which is what switches the scatter on" },

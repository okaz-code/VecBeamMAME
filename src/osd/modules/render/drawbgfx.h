...
public:
	...
	struct vec_slider_cache
	{
		...
		float z_rise_tau = 0.0f;

		// Read once per frame like everything above. These were plain slider_value() lookups -
		// a string concat plus a linear scan of every chain slider, per name, per frame.
		float ambient_level = 0.0f;
		float ambient_sdr_trim = 1.0f;
		float beam_flash_gain = 1.0f;
		float beam_flash_ms = 0.0f;
		float beam_peak_nits = 1000.0f;
		float beam_peak_ratio = 0.0f;
		float beam_window = 0.0f;
		float beam_window_adaptive = 1.0f;
		float beam_window_latch_ms = 0.0f;
		float beam_window_scale = 1.0f;
		float cap_no_persist = 0.0f;
		float edge_glow = 0.0f;
		float edge_glow_length = 80.0f;
		float edge_glow_persist = 120.0f;
		float edge_glow_sensitivity = 1.0f;
		float edge_glow_threshold = 0.0f;
		float edge_glow_width = 10.0f;
		float flicker_blue_depth = 1.0f;
		float flicker_buckets = 6.0f;
		float flicker_green_depth = 1.0f;
		float flicker_period_ms = 16.7f;
		float flicker_red_depth = 1.0f;
		float flicker_thresh_ms = 12.0f;
		float glow_fbo_scale = 1.0f;
		float halo_quad_extent = 3.5f;
		float hdr_beam_floor_nits = 0.0f;
		float hdr_glow_stability = 1.0f;
		float hdr_headroom_override = 0.0f;
		float hdr_peak_target_nits = 0.0f;
		float hdr_rolloff_max = 1.3f;
		float hdr_sat_protect = 0.0f;
		float hdr_shoulder_start = 0.85f;
		float join_extend = 1.0f;
		float mglow_coefficient = 0.0f;
		float mglow_coverage_full = 0.85f;
		float mglow_min_distance = 0.30f;
		float overlay_ambient_light = 0.50f;
		float overlay_color_dark_level = 0.15f;
		float overlay_color_density = 2.5f;
		float overlay_color_glow = 1.10f;
		float overlay_color_highlight_bleach = 0.0f;
		float overlay_color_highlight_curve = 0.55f;
		float overlay_color_highlight_knee = 0.15f;
		float overlay_diffusion_radius = 12.0f;
		float overlay_diffusion_shape = 0.4f;
		float overlay_shadow = 0.0f;
		float overlay_shadow_azimuth = 45.0f;
		...
		float overlay_white_transmission = 0.65f;
		float phosphor_curve = 1.2f;
		float phosphor_energy_decay = 0.0f;
		float phosphor_half_ms = 42.0f;
		float phosphor_hold_ms = 0.0f;
		float phosphor_total_ms = 500.0f;
		float point_roundness = 0.4f;
		float ray_count = 6.0f;
		float room_ambient = 1.0f;
		float sdr_beam_level = 1.0f;
		float sdr_rolloff_ceiling = 1.0f;
		float sdr_rolloff_knee = 1.0f;
		float sdr_shadow_curve = 1.0f;
		float vertex_dwell_energy = 0.0f;
		float vertex_dwell_width = 1.0f;   // Z rise-time (us); 0 = off. Dims brief-dwell dots (see put_analytic_line).
...
private:
	...
	float m_logged_glow_compensation = -1.0f;

...
float tube_active_amount()
{
	...
	float geometry = abs(u_tube_distortion.x) + abs(u_tube_cubic_distortion.x)
		...
		+ abs(1.0 - u_tube_face_scale.x) + u_tube_distort_fit.x;
...
vec2 distort_centered(vec2 p, float amount, float cubic_amount)
{
	...
	// Corner fit: the same factor evaluated at the corner radius, applied to BOTH axes. It is the
	// largest factor anywhere on the boundary, so dividing by it maps the corner to itself and
	// everything else inside - the rect is fully covered for any distortion amount.
	float rc2 = rx2 + ry2;
	float fit_c = 1.0 + rc2 * (amount + cubic * sqrt(rc2));
	float fit_blend = clamp(u_tube_distort_fit.x, 0.0, 1.0);
	vec2 fit = mix(vec2(fit_x, fit_y), vec2_splat(fit_c), fit_blend);
	physical *= vec2(f / safe_distortion_divisor(fit.x), f / safe_distortion_divisor(fit.y));
